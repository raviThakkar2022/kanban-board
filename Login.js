...
function LoginForm() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          ...
          const response = await axios.get(`http://localhost:3001/users?username=${usernameEmail}&password=${password}`);
    ...
    <div className="login-form-container">
      ...
      <form className="login-form" onSubmit={handleSubmit}>
        <label className="login-label">
          Username/Email:
          <input className="login-input" type="text" value={usernameEmail} onChange={(e) => setUsernameEmail(e.target.value)} required />
        </label>
        <br />
        <label className="login-label">
          Password:
